...
test.describe('Remote Episodes Drawer', () => {
  ...
  /**
   * Test: Season Picker Selection Persists After Manual Selection
   * 
   * This test verifies:
   * 1. Master and slave connect via QR code flow
   * 2. Master starts playing a TV show episode
   * 3. Master opens episodes drawer
   * 4. Master selects a different season
   * 5. The season selection persists in the picker (does not reset)
   */
  test('season picker selection persists after manual selection', async ({ browser }) => {
    // ===== MASTER BROWSER SETUP =====
    const masterPage = await masterContext.newPage();
    console.log('=== MASTER: Opening application ===');
    ...
    if (await episodesButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // ===== STEP 7: VERIFY INITIAL SEASON SELECTION =====
      console.log('\n=== STEP 7: Verify Initial Season Selection ===');
      
      // Find the season Select element
      const seasonSelect = masterPage.locator('#episodes-drawer').locator('.MuiSelect-select');
      await seasonSelect.waitFor({ state: 'visible', timeout: 5000 });
      
      // Get initial selected season
      const initialSeasonValue = await seasonSelect.textContent();
      console.log(`Initial season selected: "${initialSeasonValue}"`);
      
      // ===== STEP 8: SELECT A DIFFERENT SEASON =====
      console.log('\n=== STEP 8: Select a Different Season ===');
      
      // Click on the season select to open the dropdown
      await seasonSelect.click();
      await masterPage.waitForTimeout(500);
      
      // Get all season options
      const seasonOptions = masterPage.locator('#episodes-drawer .MuiMenu-item');
      const optionCount = await seasonOptions.count();
      console.log(`Found ${optionCount} season options`);
      
      if (optionCount > 1) {
        // Select the second season (different from initial)
        const secondSeason = seasonOptions.nth(1);
        const secondSeasonText = await secondSeason.textContent();
        console.log(`Selecting different season: "${secondSeasonText}"`);
        await secondSeason.click();
        await masterPage.waitForTimeout(500);
        
        // ===== STEP 9: VERIFY SELECTION PERSISTS =====
        console.log('\n=== STEP 9: Verify Season Selection Persists ===');
        
        // Click on an episode (to potentially trigger nowPlayingItem changes)
        const episodeList = masterPage.locator('#episodes-drawer li[role="option"], [data-testid*="episode"], li >> text=/^\\d+\\./');
        const episodeCount = await episodeList.count();
        
        if (episodeCount > 0) {
          console.log(`Found ${episodeCount} episodes, clicking first one...`);
          await episodeList.first().click();
          await masterPage.waitForTimeout(2000);
        }
        
        // Re-open the drawer to check if selection persisted
        const episodesButtonAgain = masterPage.locator('button:has-text("Episodes"), button[aria-label*="episodes" i]');
        if (await episodesButtonAgain.isVisible({ timeout: 5000 }).catch(() => false)) {
          await episodesButtonAgain.click();
          await masterPage.waitForTimeout(1000);
        }
        
        // Check if the season is still selected
        const seasonSelectAfter = masterPage.locator('#episodes-drawer').locator('.MuiSelect-select');
        await seasonSelectAfter.waitFor({ state: 'visible', timeout: 5000 });
        
        const finalSeasonValue = await seasonSelectAfter.textContent();
        console.log(`Final season value: "${finalSeasonValue}"`);
        
        // Verify the season selection persisted
        if (finalSeasonValue === secondSeasonText) {
          console.log('SUCCESS: Season selection persisted!');
        } else {
          console.log(`FAILURE: Season changed from "${secondSeasonText}" to "${finalSeasonValue}"`);
        }
        
        // Take screenshot for debugging
        await masterPage.screenshot({ path: 'season-picker-final-state.png', fullPage: true });
        console.log('Screenshot saved: season-picker-final-state.png');
      } else {
        console.log('Only one season found - skipping season selection test');
      }
    } else {
      console.log('Episodes button not visible - test may have reached different screen');
      // Take screenshot for debugging
