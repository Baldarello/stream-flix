...
import type {MediaItem, PlayableItem, MediaLink} from '../types.ts';
...
class RemoteStore {
    // ===== PRIVATE FIELDS =====
    private _isSmartTV = false;
    private _isSmartTVPairingVisible = false;
    private _isRemoteMaster = false;
    private _slaveId: string | null = null;
    private _slaveShortCode: string | null = null;
    private _isRemoteMasterConnected = false;
    private _hasLoadedInitialData = false;
    private _masterReconnectAttempts = 0;
    private _masterReconnectTimer: number | null = null;
    private _isReconnecting = false;
    private _remoteSlaveState: RemoteSlaveState | null = null;
    private _remoteSelectedItem: MediaItem | null = null;
    private _isRemoteDetailLoading = false;
    private _remoteAction: { type: string; payload?: any; id: number } | null = null;
    private _remoteFullItem: MediaItem | null = null;
    private _isRemoteFullItemLoading = false;
    private _isIntroSkippableOnSlave = false;
    private _shouldAutoFullscreen = false;
    @observable _knownSlaves: { id: string; name: string; lastSeen: number; isOnline?: boolean; shortCode?: string }[] = [];
    private _missedPings = 0;
    private _connectionHealth: 'good' | 'degraded' | 'poor' = 'good';
    private _pingInterval: number | null = null;
    private _lastPingTime: number | null = null;
    private _isMediaSyncModalOpen = false;
    private _mediaSyncTargetSlaveId: string | null = null;
    @observable __masterUiActiveView: ActiveView = 'Home';
    @observable __masterUiSelectedItem: MediaItem | null = null;
    private _isQRScannerOpen = false;
    ...
    @computed get remoteNextEpisode() {
    ...
    @computed get remotePreviousEpisode() {
        const nowPlaying = this.remoteSlaveState?.nowPlayingItem;
        if (!nowPlaying || !('episode_number' in nowPlaying) || !this.remoteFullItem?.seasons) return null;
    ...
    selectMedia = async (item: MediaItem, context: 'remoteControl' | 'cacheOnly' = 'remoteControl') => {
        if (context === 'remoteControl') {
            ...
            try {
                let fullItemDetails: MediaItem = mediaStore.cachedItems.get(item.id) || item;
    ...
    sendRemoteCommand = (command: { command: string; item?: PlayableItem; time?: number }) => {
    ...
    playRemoteItem = async (item: PlayableItem) => {
        ...
        const allLinks = (item as any).video_urls || await mediaStore.getLinksForMedia(mediaId);
        ...
        const showId = 'show_id' in item ? (item as any).show_id : item.id;
    ...
    handleRemoteCommand = (payload: { command: string; item?: PlayableItem; time?: number; slaveId?: string }) => {
        ...
        switch (command) {
            ...
            case 'skip_intro': {
                ...
                if (video && activeItem) {
                    if ('intro_end_s' in activeItem && (activeItem as any).intro_end_s && (activeItem as any).intro_end_s > (activeItem as any).intro_start_s) {
                        video.currentTime = (activeItem as any).intro_end_s;
                    } else {
                        const showId = 'show_id' in activeItem ? (activeItem as any).show_id : (activeItem as any).id;
        ...
        runInAction(() => {
            ...
            switch (type) {
                ...
                case 'connection-established':
                    if ((payload as any)?.role === 'master') {
                        console.log(`[RemoteStore] connection-established: masterId=${(payload as any).masterId}, tvId=${(payload as any).tvId}`);
                        runInAction(() => {
                            this.isRemoteMasterConnected = true;
                            this.slaveId = (payload as any).tvId;
                        });
                    ...
                    } else if ((payload as any)?.role === 'slave') {
                        console.log(`[RemoteStore] connection-established: slave connected to master ${(payload as any).masterId}`);
                        runInAction(() => {
                ...
                case 'connection-error':
                    console.log(`[RemoteStore] connection-error: ${(payload as any).error} - ${(payload as any).message}`);
                    if ((payload as any).error === 'TV_BUSY') {
                        this.showSnackbar('notifications.slaveBusy', 'warning', true);
                    } else if ((payload as any).error === 'TV_NOT_FOUND') {
                        this.showSnackbar('notifications.slaveNotFound', 'error', true);
                        this.handleReconnectFailed((payload as any).shortCode);
                    } else if ((payload as any).error === 'TV_OFFLINE') {
                        this.showSnackbar('notifications.slaveOffline', 'warning', true);
                    } else {
                        this.showSnackbar((payload as any).message || 'Connection failed', 'error', true);
                    }
                    break;
                case 'connection-terminated':
                    console.log(`[RemoteStore] connection-terminated: target=${(payload as any).target}, reason=${(payload as any).reason}`);
                    if ((payload as any).target === 'slave') {
                        this.setSlaveOnlineStatus(this.slaveId, false);
                        this.handleSlaveDisconnected((payload as any).reason !== 'slave-initiated');
                        this.stopPingInterval();
                    } else if ((payload as any).target === 'master') {
                        runInAction(() => {
                            this.isRemoteMasterConnected = false;
                        });
                        this.knownSlaves.forEach(slave => {
                ...
                case 'playback-status':
                    ...
                    break;
                case 'heartbeat-ping':
                    console.log("[RemoteStore] Received heartbeat-ping from", (payload as any).source);
                    if (this.slaveId) {
                        websocketService.sendMessage({
                            type: 'heartbeat-pong',
                ...
                case 'command-error':
                    ...
                    this.showSnackbar((payload as any).message || 'An error occurred', 'error', true);
                    break;
            }
        });
    };
    ...
    handleReconnectFailed = (shortCodeAvailable?: string) => {
