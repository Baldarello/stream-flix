...
describe('expandPattern', () => {
    ...
    it('honors a custom startNum range', () => {
        ...
        expect(out).toEqual([
            'https://cdn.example.com/ep100.mp4',
            'https://cdn.example.com/ep101.mp4',
        ]);
    });

    it('returns an empty array for invalid ranges', () => {
        expect(
            expandPattern({ pattern: 'https://x/[@EP]', padding: 2, start: 5, end: 3 })
        ).toEqual([]);
    });

    it('returns an empty array when the pattern lacks the placeholder', () => {
        expect(
            expandPattern({ pattern: 'https://x/static.mp4', padding: 2, start: 1, end: 2 })
        ).toEqual([]);
    });
});
