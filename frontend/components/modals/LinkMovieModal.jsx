...
const LinkMovieModal = observer(() => {
    ...
    const preferredOriginForMovie = item ? preferredSources.get(item.id) : null;
    ...
    return (
        ...
        <Modal open={isLinkMovieModalOpen} onClose={closeLinkMovieModal}>
            <Box sx={style}>
                <IconButton onClick={closeLinkMovieModal} sx={{ position: 'absolute', right: 8, top: 8 }}>
                    <CloseIcon />
                </IconButton>
                <Typography variant="h6" component="h2" noWrap>
                    {t('linkMovieModal.title', { title: item.title })}
                </Typography>

                <Stack spacing={2} sx={{ mt: 3, flex: 1, overflow: 'hidden' }}>
                    <Paper variant="outlined" sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>
                            {t('linkMovieModal.addLink')}
                        </Typography>
                        <Stack spacing={2}>
                            <TextField
                                label={t('linkMovieModal.url')}
                                value={newUrl}
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => <TextField {...params} label={t('linkMovieModal.label')} />}
                            />
                            <Button
