...
import { watchTogetherStore } from '../../store/watchTogetherStore.js';
import {
    Alert,
    Box,
    Button,
...
import { searchShow } from '../../services/apiCall.jsx';
...
const WatchTogetherModal = observer(() => {
    ...
    const [selectedSeason, setSelectedSeason] = useState(
        () => (itemForModal && 'seasons' in itemForModal && itemForModal.seasons?.[0]?.season_number) || 1
    );
    ...
    const currentSeason =
        itemForModal && 'seasons' in itemForModal && itemForModal.seasons
    ...
    const renderInitialView = () => {
        ...
        // Safely check for 'media_type' property before accessing it, is a union type
        // and one of its types (Episode) does not have this property.
        const isMovieSelected =
            watchTogetherSelectedItem &&
            'media_type' in watchTogetherSelectedItem &&
            watchTogetherSelectedItem.media_type === 'movie';
            ...
            <Stack spacing={3}>
                ...
                {!isJoiningViaLink && (
                    <>
                                    ...
                                    {itemForModal.seasons && itemForModal.seasons.length > 1 && (
                                        <FormControl size="small" sx={{ minWidth: 150 }}>
                                            ...
                                            <Select
                                                ...
                                                label={t('watchTogether.season')}
                                                onChange={(e) => setSelectedSeason(Number(e.target.value))}
                                            >
                                                {itemForModal.seasons.map((s) => (
                        ...
                        <Button variant="contained" size="large" onClick={() => createRoom(username)} disabled={!canCreateRoom}>
                ...
                <Stack direction="row" spacing={1}>
                    ...
                    <Button
                        variant="outlined"
                        onClick={() => joinRoom(inputRoomId, username)}
                        disabled={!inputRoomId.trim() || !username.trim()}
                    >
                ...
                {searchResults.map((item) => (
                    <ListItemButton key={item.id} onClick={() => handleSelectNewContent(item)}>
                        ...
                        <ListItemText
                            ...
                            secondary={item.first_air_date?.substring(0, 4) || item.release_date?.substring(0, 4)}
                        />
                    </ListItemButton>
                ))}
            </List>
        ...
        if (!itemForModal) {
            return (
                <Box sx={{ textAlign: 'center', p: 4 }}>
                    <CircularProgress />
                    <Typography sx={{ mt: 2 }}>Sincronizzazione contenuti...</Typography>
                </Box>
            );
        }
        ...
        return (
            <Stack spacing={2} sx={{ overflow: 'hidden', flex: 1 }}>
                <Typography variant="h6" component="h2" noWrap>
                    {t('watchTogether.roomTitle', { title: roomTitle })}
                </Typography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'rgba(255,255,255,0.1)', p: 1, borderRadius: 1 }}>
                    <Typography
                        variant="h5"
                        component="p"
                        sx={{ flexGrow: 1, textAlign: 'center', letterSpacing: '0.2rem', fontFamily: 'monospace' }}
                    >
                        {roomId}
                    </Typography>
                    {isHost && (
                        ...
                        <Tooltip title={t('watchTogether.changeCode')}>
                            <IconButton onClick={changeRoomCode}>
                                <RefreshIcon />
                            </IconButton>
                        </Tooltip>
                    )}
                            ...
                            {isDetailLoading ? (
                                ...
                                currentSeason?.episodes.map((ep) => (
                                    <ListItemButton
                                        ...
                                        selected={
                                            watchTogetherSelectedItem &&
                                            'episode_number' in watchTogetherSelectedItem &&
                                            watchTogetherSelectedItem.id === ep.id
                                        }
                                        onClick={() => handleSelectEpisode(ep)}
                                    >
                                        <ListItemText primary={`${ep.episode_number}. ${ep.name}`} />
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                        {p.id === mediaStore.myClientId ? (
                                            <Tooltip title={t('profileDrawer.changeName') || 'Change name'}>
                                                <IconButton
                                                    edge="end"
                                                    aria-label={t('profileDrawer.changeName') || 'Change name'}
                                                    size="small"
                                                    onClick={() => handleNameChangeClick({ id: p.id, name: p.name })}
                                                >
                                            ...
                                            <Tooltip title={t('chat.makeHost')}>
                                                <IconButton
                                                    edge="end"
                                                    aria-label={t('chat.makeHost')}
                                                    onClick={() => handleTransferHostClick({ id: p.id, name: p.name })}
                                                >
                                                    <SwapHorizIcon />
                                                </IconButton>
                                            </Tooltip>
                ...
                {isHost && watchTogetherSelectedItem && (
                    <Stack direction="row" spacing={2}>
                        <Button variant="outlined" fullWidth onClick={() => setIsChangingContent(true)}>
                            {t('watchTogether.changeContent')}
                        </Button>
                        <Button
                            variant="contained"
                            fullWidth
                            color="primary"
                            onClick={() => mediaStore.startPlayback(watchTogetherSelectedItem)}
                        >
    ...
    return (
        <>
            <Modal open={watchTogetherModalOpen} onClose={handleClose} aria-labelledby="watch-together-modal-title">
                <Box sx={style}>
                    <IconButton
                        aria-label="close"
                        onClick={handleClose}
            ...
            <Dialog open={transferConfirmOpen} onClose={handleCancelTransfer} aria-labelledby="transfer-host-dialog-title">
            ...
            <Dialog open={nameEditOpen} onClose={handleCancelNameChange} aria-labelledby="name-edit-dialog-title">
                <DialogTitle id="name-edit-dialog-title">{t('profileDrawer.changeName') || 'Change Name'}</DialogTitle>
                <DialogContent>
                    <TextField
                        autoFocus
                        margin="dense"
