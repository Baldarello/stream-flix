                ...
                {item.seasons && item.seasons.length > 0 && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                        {availableLanguages.length > 1 && (
                            <FormControl sx={{ minWidth: 120 }} size="small">
                                ...
                                <Select
                                    ...
                                    onChange={(e) => onLanguageChange(item.id, e.target.value)}
                                    sx={holoFieldSx}
                                >
                        ...
                        <TextField
                            ...
                            size="small"
                            value={introDuration}
                            onChange={onIntroDurationChange}
                        ...
                        <FormControl sx={{ minWidth: 120 }} size="small">
                            ...
                            <Select
                                ...
                                onChange={(e) => onSeasonChange(item.id, Number(e.target.value))}
                                sx={holoFieldSx}
                            >
...
export const EpisodeListView = observer(({ currentSeason, isDetailLoading, languageFilter, typeFilter, item }) => {
    const { t } = useTranslations();
    if (isDetailLoading) {
        return (
...
const DetailView = observer(() => {
    ...
    const languageFilter =
        currentPreferences.language && availableLanguages.includes(currentPreferences.language)
            ? currentPreferences.language
            : availableLanguages[0];
    const typeFilter =
        currentPreferences.type && availableTypes.includes(currentPreferences.type) ? currentPreferences.type : availableTypes[0];
    ...
    return (
        ...
        >
            ...
            <Box
                data-component="detail-view-scroll"
                sx={{
                    position: 'relative',
                    height: '100%',
                    width: '100%',
