            ...
            return (
                <>
                    ...
                    <Stack direction="row" sx={{ justifyContent: 'center', alignItems: 'center', gap: 2 }}>
                        <IconButton onClick={handleRewind10} color="inherit" disabled={!!roomId && !isHost} size="medium">
                            <Replay10Icon />
                        </IconButton>
                        <IconButton
                        ...
                        >
                            {playerState.isPlaying ? <PauseIcon fontSize="medium" /> : <PlayArrowIcon fontSize="medium" />}
                        </IconButton>
                        <IconButton onClick={handleForward10} color="inherit" disabled={!!roomId && !isHost} size="medium">
                            <Forward10Icon />
                        </IconButton>
                        <Typography variant="caption" sx={{ fontFamily: 'monospace', minWidth: 85, textAlign: 'center' }}>
                            {formatTime(playerState.currentTime)} / {formatTime(playerState.duration)}
                        </Typography>
                        <IconButton onClick={handleToggleFullScreen} color="inherit" disabled={!!roomId && !isHost} size="medium">
                ...
                <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
                    <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
                        <Tooltip title={t('videoPlayer.skipBack10') || 'Rewind 10s'}>
                            <IconButton onClick={handleRewind10} color="inherit" disabled={!!roomId && !isHost}>
                        ...
                        <Tooltip title={t('videoPlayer.forward10') || 'Forward 10s'}>
                            <IconButton onClick={handleForward10} color="inherit" disabled={!!roomId && !isHost}>
                                <Forward10Icon fontSize="large" />
                            </IconButton>
                        </Tooltip>
                        <Tooltip title={t('videoPlayer.skipIntro') || 'Skip Intro/Credits'}>
                            <IconButton onClick={handleSkipIntro} color="inherit" disabled={!!roomId && !isHost}>
