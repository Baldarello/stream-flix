...
const DetailView: React.FC = observer(() => {
  const { myList, showIntroDurations, setShowIntroDuration, episodeProgress, selectedSeasons, setSelectedSeasonForShow, showFilterPreferences, setShowFilterPreference } = mediaStore;
  ...
  useEffect(() => {
    const loadItem = async () => {
        if (id) {
            setIsLoading(true);
            const fullItemDetails = await mediaStore.fetchFullMediaDetails(Number(id));
            setItem(fullItemDetails);
            setIsLoading(false);
        }
    };
    loadItem();
  ...
  if (isLoading || !item) {
    return (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', bgcolor: 'rgba(0,0,0,0.8)' }}>
  ...
  const handlePlayClick = async (itemToPlay: PlayableItem) => {
    const playableItem = await mediaStore.prepareItemForPlayback(itemToPlay);
    if (playableItem) {
  ...
  return (
    <Box sx={{ position: 'fixed', inset: 0, zIndex: 1200, animation: 'fadeIn 0.5s ease-in-out' }}>
        {backgroundImage ? (
            <Box sx={{ position: 'absolute', inset: 0, backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(20px) brightness(0.5)', transform: 'scale(1.1)' }} />
        ) : (
             <Box sx={{ position: 'absolute', inset: 0, bgcolor: '#141414' }} />
        )}

        <IconButton
            onClick={() => navigate(-1)}
            aria-label={t('detail.close')}
            sx={{ position: 'absolute', top: 16, right: 16, zIndex: 1300, bgcolor: 'rgba(0,0,0,0.5)', transform: 'scale(1.2)', transition: 'transform 0.3s ease, background-color 0.3s ease', '&:hover': { bgcolor: 'rgba(0,0,0,0.8)', transform: 'scale(1.3) rotate(90deg)' } }}
        >
            <CloseIcon />
        </IconButton>

        <Box sx={{ position: 'relative', height: '100%', overflowY: 'auto' }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '350px 1fr' }, gap: 4, p: { xs: 2, md: 8 }, pt: { xs: 8, md: 8 }, minHeight: '60vh', alignItems: 'center' }}>
                {item.poster_path ? (
                    <CardMedia component="img" image={item.poster_path} alt={title} sx={{ width: '100%', maxWidth: '350px', aspectRatio: '2/3', borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', justifySelf: 'center' }} />
                ) : (
                    <Box sx={{ width: '100%', maxWidth: '350px', aspectRatio: '2/3', borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', justifySelf: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                        <TheatersIcon color="disabled" sx={{ fontSize: '6rem' }} />
                    </Box>
                )}
                <Stack spacing={2} sx={{ p: { xs: 2, md: 4 }, bgcolor: 'background.paper', backdropFilter: 'blur(10px)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.1)' }}>
                    <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                        ...
                        {item.media_type === 'movie' && (
                             <Tooltip title={t('detail.linkEpisodesTooltip')}>
                                <IconButton onClick={() => mediaStore.openLinkMovieModal(item)}><LinkIcon /></IconButton>
                            </Tooltip>
                        )}
                    </Stack>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ pt: 2, alignItems: { xs: 'stretch', sm: 'center' } }}>
                        <Button variant="contained" color="inherit" startIcon={<PlayArrowIcon />} size="large" sx={{ bgcolor: 'white', color: 'black', '&:hover': { bgcolor: 'white', boxShadow: '0 0 15px 5px rgba(255, 255, 255, 0.5)' } }} onClick={() => handlePlayClick(item)}>
                            {t('detail.play')}
                        </Button>
                        <Tooltip title={listActionLabel}>
                            <IconButton onClick={() => mediaStore.toggleMyList(item)} aria-label={listActionLabel} sx={{ border: '2px solid rgba(255,255,255,0.7)', color: 'white', alignSelf: { xs: 'flex-start' }, width: 48, height: 48, '&:hover': { borderColor: 'white', boxShadow: `0 0 10px ${getGlowColor()}` } }}>
                                {isInMyList ? <CheckIcon /> : <AddIcon />}
                            </IconButton>
                        </Tooltip>
                        <Button variant="outlined" startIcon={<GroupIcon />} size="large" onClick={() => mediaStore.openWatchTogetherModal(item)} sx={{ borderColor: 'rgba(255,255,255,0.7)', color: 'white', '&:hover': { borderColor: 'white', bgcolor: 'rgba(255,255,255,0.1)', boxShadow: `0 0 10px ${getGlowColor()}` } }}>
                ...
                <Box sx={{ p: { xs: 2, md: 8 }, pt: 0 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, flexWrap: 'wrap', gap: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <Tooltip title={t('detail.linkEpisodesTooltip')}>
                                <IconButton onClick={() => mediaStore.openLinkEpisodesModal(item)}><LinkIcon /></IconButton>
                            </Tooltip>
                        </Box>
                        {item.seasons && item.seasons.length > 0 && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                                ...
                                <TextField
                                    label={t('detail.introDuration')} type="number" variant="outlined" size="small" value={introDuration} onChange={handleIntroDurationChange} onFocus={(event) => event.target.select()} sx={{ width: 150 }}
                                ...
                                <FormControl sx={{ minWidth: 120 }} size="small">
                                    ...
                                    <Select labelId="season-select-label" value={selectedSeason} label={t('detail.season')} onChange={(e) => setSelectedSeasonForShow(item.id, Number(e.target.value))} sx={{ bgcolor: 'rgba(20, 20, 30, 0.7)', '.MuiOutlinedInput-notchedOutline': { borderColor: 'rgba(255,255,255,0.2)' } }}>
                    ...
                    <List>
                        {(currentSeason?.episodes || []).map((episode: Episode) => {
                            ...
                            return (
                            <ListItem
                                key={episode.id} disablePadding
                                secondaryAction={ <Tooltip title={isWatched ? t('detail.markAsUnwatched') : t('detail.markAsWatched')}><IconButton edge="end" aria-label={isWatched ? t('detail.markAsUnwatched') : t('detail.markAsWatched')} onClick={() => mediaStore.toggleEpisodeWatchedStatus(episode.id)}>{isWatched ? <CheckCircleIcon color="success" /> : <CheckCircleOutlineIcon />}</IconButton></Tooltip> }
                                sx={{ mb: 1.5, bgcolor: 'rgba(20, 20, 30, 0.6)', borderRadius: 2, transition: 'background-color 0.2s, transform 0.2s', '&:hover': { bgcolor: 'rgba(40, 40, 50, 0.8)', transform: hasPlayableLinks ? 'scale(1.02)' : 'none', }, '& .MuiListItemSecondaryAction-root': { right: '16px' } }}
                            >
                                <ListItemButton
                                    disabled={!hasPlayableLinks}
                                    onClick={() => handlePlayClick({ ...episode, show_id: item.id, show_title: item.title || item.name || '', backdrop_path: item.backdrop_path, season_number: currentSeason.season_number })}
                                    sx={{ p: 2, borderRadius: 2, pr: '64px', opacity: hasPlayableLinks ? 1 : 0.5 }}
                                >
                                    ...
                                    <Box sx={{ position: 'relative', width: 150, aspectRatio: '16/9', mr: 2, flexShrink: 0, overflow: 'hidden', borderRadius: 1 }}>
                                        {episode.still_path ? <CardMedia component="img" image={episode.still_path} alt={`Scena da ${episode.name}`} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Box sx={{ width: '100%', height: '100%', bgcolor: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TheatersIcon color="disabled" sx={{ fontSize: '3rem' }} /></Box>}
                                        {watchedPercentage > 0 && !isWatched && (<LinearProgress variant="determinate" value={watchedPercentage} color="primary" sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 4 }} />)}
                                        {isWatched && (<Box sx={{ position: 'absolute', inset: 0, bgcolor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CheckCircleIcon color="success" sx={{ fontSize: '3rem' }} /></Box>)}
                                    </Box>
